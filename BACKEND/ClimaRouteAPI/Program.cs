...
{
    options.FormatterName = Microsoft.Extensions.Logging.Console.ConsoleFormatterNames.Systemd;
});
...
// Robust fallback: If connection string is empty or contains unresolved Docker variables, build it from individual env vars
if (string.IsNullOrEmpty(connectionString) || connectionString.Contains("${"))
{
    var dbHost = builder.Configuration["DB_HOST"] ?? "db";
    var dbPort = builder.Configuration["DB_PORT"] ?? "5432";
    var dbName = builder.Configuration["POSTGRES_DB"] ?? "climaroute";
    var dbUser = builder.Configuration["POSTGRES_USER"] ?? "postgres";
    var dbPass = builder.Configuration["POSTGRES_PASSWORD"] ?? "postgres";
...
app.MapGet("/api/more-health", async (AppDbContext db, IHttpClientFactory clientFactory) =>
{
